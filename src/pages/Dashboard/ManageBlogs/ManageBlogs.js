...
const ManageBlogs = () => {
  ...
  const handleDeleteBlogs = (id) => {
    ...
    if (proceed) {
      const url = `https://still-oasis-67632.herokuapp.com/allblogs/${id}`;
      fetch(url, {
        method: "DELETE",
      })
        .then((res) => res.json())
        ...
        <div>
          <ul>
            <li>
              ...
              <button onClick={() => handleUpdateBlogs(blog._id)}>
                Update
              </button>
              <button onClick={() => handleDeleteBlogs(blog._id)}>
                delete
              </button>
            </li>
          </ul>
